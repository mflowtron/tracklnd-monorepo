...
query "auth/signup" verb=POST {
  ...
  stack {
    ...
    // Imported users may already exist without a password.
    // If so, send a claim link instead of blocking with a generic error.
    conditional {
      if ($user != null) {
        conditional {
    ...
    // Default all users to the viewer role
    db.add user_role {
